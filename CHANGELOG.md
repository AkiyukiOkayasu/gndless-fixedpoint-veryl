...
- `Q4_23` presetを追加（module境界の統一format用途。±1.0の信号を±8.0の範囲で扱い、24bit PCMと同じ2^-23グリッドを保つ）

## [0.2.1] - 2026-08-06

### Added

- Q1.17を追加（halfband係数など乗算器の係数用途）
...
- ADAT PCM境界用の24bit `Q1_23` preset、format付きvalue接続用の`FixedPointValue::<FORMAT>` interface、Q1.23/Q2.23間を明示接続する`FormatConverter`を追加

### Changed

- 破壊的変更: `SignedFixedPointFormat::Sample`と`convert_sample`を削除。public value interfaceは`FixedPointValue::<FORMAT>`、内部payloadは`FORMAT::Raw`へ整理
